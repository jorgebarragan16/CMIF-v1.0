...
$req = (strlen($fecha)*strlen($tipo)*strlen($sitio)*strlen($info)) or die('"<script> alert("Digite TODOS los campos")</script>" "<script> window.location="Enviar_Formato_Solicitud_Docentes.php"</script>');

//Ingresamos la info a la base de datos

mysql_query("INSERT INTO solicitud VALUES ('','$fecha','$tipo','$sitio','$info',1,'$id_d')",$link) or die('"<script> alert("'.mysql_error().'")</script>" "<script> window.location="Enviar_Formato_Solicitud_Docentes.php"</script>');

echo '<script> alert("Solicitud Registrada");</script>';
echo '<script> window.location="Pagina_Principal_Docentes.php"; </script>';
?>
</body>
</html>
